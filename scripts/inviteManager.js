...
class InviteManager {
    ...
    async pushLogToGitHub() {
        try {
            const git = simpleGit();
            const today = new Date().toISOString().split('T')[0];
            const logFileName = `invites-${today}.log`;
            
            await this.log('GIT', 'Pushing log file to GitHub');
            
            // Add and commit only the current log file
            await git.add(path.join('logs', logFileName));
            await git.commit(`[Automated] Log update for ${today}`);
            await git.push('origin', 'master');
            
            await this.log('GIT', 'Successfully pushed log file to GitHub');
        } catch (error) {
            await this.log('GIT_ERROR', 'Failed to push log to GitHub', { error: error.message });
            console.error('Failed to push log to GitHub:', error.message);
        }
    }
    ...
    async inviteUsers(searchTerm, maxInvites = 50) {
        ...
        // Push log file to GitHub after completing invitations
        await this.pushLogToGitHub();
